...
def countTilesEnclosedByLoop(grid):
	loopTiles = getLoopTiles(grid)
	result = 0

	for row in range(len(grid)):
		lastVerticalWall = ''
		oddVerticalWalls = False

		for col in range(len(grid[row])):
			if (row, col) in loopTiles:
				if grid[row][col] in '|LJ7F':
					if (lastVerticalWall + grid[row][col]) not in ['L7', 'FJ']:
						oddVerticalWalls = not oddVerticalWalls

					lastVerticalWall = grid[row][col]
			else:
				result += oddVerticalWalls
...
if __name__ == '__main__':
	...
	print(countTilesEnclosedByLoop(grid))
